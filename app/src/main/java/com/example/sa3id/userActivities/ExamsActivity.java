...
public class ExamsActivity extends BaseActivity {
    ...
    private void loadUserSettings(String userId) {
        ...
        FirebaseDatabase.getInstance(FIREBASE_REALTIME_LINK).getReference().child("users").child(userId).addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(DataSnapshot dataSnapshot) {
                if (dataSnapshot.exists()) {
                    ...
                    new CustomAlertDialog(ExamsActivity.this).show("لم يتم العثور على إعدادات المستخدم", R.drawable.baseline_error_24);
                }
            }
            ...
            public void onCancelled(DatabaseError databaseError) {
                progressBar.setVisibility(View.GONE);
                new CustomAlertDialog(ExamsActivity.this).show("خطأ في تحميل إعدادات المستخدم: " + databaseError.getMessage(), R.drawable.baseline_error_24);
            }
        });
    }
    ...
    private void loadExams() {
        if (userSubjects.isEmpty()) {
            progressBar.setVisibility(View.GONE);
            new CustomAlertDialog(this).show("يرجى اختيار المواضيع أولاً", R.drawable.baseline_error_24);
        ...
        examManager.getExamsForUserSubjects(userSubjects, new ExamManager.ExamCallback() {
            ...
            public void onError(String error) {
                progressBar.setVisibility(View.GONE);
                new CustomAlertDialog(ExamsActivity.this).show("خطأ في تحميل الامتحانات: " + error, R.drawable.baseline_error_24);
            }
        });
    }
    ...
    protected void onResume() {
        ...
        LocalBroadcastManager.getInstance(this).registerReceiver(settingsReceiver, new IntentFilter(SettingsFragment.ACTION_NOTIFICATION_SETTINGS_CHANGED));
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
        ...
        if (requestCode == NOTIFICATION_PERMISSION_CODE) {
            if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                new CustomAlertDialog(this).show("تم منح إذن الإشعارات", R.drawable.baseline_check_circle_24);
            } else {
                new CustomAlertDialog(this).show("تم رفض إذن الإشعارات", R.drawable.baseline_error_24);
            }
        }
    }
